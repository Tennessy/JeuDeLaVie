...
public class Main {
			...
			if (args[0].equals("-s")) {
				...
				if (args[0].equals("-c")) {
					try {
						...
						int typeMonde = Jeu.TypeMonde(args[3]);
						if (typeMonde != 0)
							System.out.println(calculerEvolution(max_duree,
									typeMonde, args[2]).toString());
						else
							System.out
									.println("Erreur dans la selection du Type de monde");
					} catch (NumberFormatException e) {
						System.out.println("La duree doit etre numerique \n");
						Main.ErrorArgs();
					}
								...
								if (f.isDirectory()) {
									...
									for (File fichier : f.listFiles()) {
										String nom = f.getName().concat(
												"/" + fichier.getName());
										listeEvo.add(calculerEvolution(
												max_duree, typeMonde, nom));
									}
	 ...
	 * 
	 * @param duree
	 *            Nombre d'evolution a effectuer.
	 * @param typeMonde
	 *            Type de monde dans lequel effectuer cette simulation.
	 * @param filename
	 *            Chemin du fichier lif contenant les caracteristiques du jeu.
	 */
	 ...
	 * 
	 * @param duree
	 *            Nombre d'evolution maximale ‡ effectuer.
	 * @param typeMonde
	 *            Type de monde dans lequel effectuer la simulation.
	 * @param filename
	 *            Chemin du fichier lif contenant les caracteristiques du jeu.
	 * @return Le type d'evolution du jeu.
	 */
	public static TypeEvolution calculerEvolution(int duree, int typeMonde,
			String filename) {
